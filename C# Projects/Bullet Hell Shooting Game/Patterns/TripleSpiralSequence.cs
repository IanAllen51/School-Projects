...
{
    internal class TripleSpiralSequence : Pattern
    {
        private Vector2 initPosition;
        ProjectileType type;

        public TripleSpiralSequence(ContentManager content, ProjectileType type, Vector2 pos)
        {
        ...
        public List<Projectile> returnList(int shotCount, Vector2 playerPos)
        {
            ...
            shotPos.X = initPosition.X - 100 * (float)Math.Cos(a);
            shotPos.Y = initPosition.Y - 100 * (float)Math.Sin(a);
            firePattern.Add(factory.Create(type, shotPos, Movements.MovementType.CUSTOM, new Vector2(-200, -100)));
